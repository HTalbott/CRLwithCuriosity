...
def update_actor_and_alpha(config, networks, transitions, training_state, key):
    def actor_loss(actor_params, critic_params, log_alpha, transitions, key, env_steps):
        ...
        # Exploration bonus: distance from batch centroid in representation
        # space.  Encourages the actor to visit novel regions of the learned
        # embedding.  Linearly annealed to zero.
        centroid = jax.lax.stop_gradient(jnp.mean(sa_repr, axis=0))
        novelty = -energy_fn(config["energy_fn"], sa_repr, centroid)

        anneal_end = config["exploration_anneal_frac"] * config["total_env_steps"]
        exploration_weight = config["exploration_coeff"] * jnp.clip(
            1.0 - env_steps / anneal_end, 0.0, 1.0
        )

        actor_loss = jnp.mean(
            jnp.exp(log_alpha) * log_prob - qf_pi - exploration_weight * novelty
        )

        return actor_loss, (log_prob, exploration_weight, jnp.mean(novelty))
    ...
    (actor_loss, (log_prob, exploration_weight, mean_novelty)), actor_grad = (
    ...
    metrics = {
        ...
        "exploration_novelty": mean_novelty,
    }
